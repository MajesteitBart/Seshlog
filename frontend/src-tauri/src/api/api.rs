...
// NOTE: Backend dependency removed - all data operations use direct SQLx queries
// Profile-related operations (get_profile, save_profile, update_profile) are deprecated
// as Meeting Companion does not require a backend server.
...
/// DEPRECATED: Profile operations are no longer supported - backend not required
#[allow(dead_code)]
#[derive(Debug, Serialize, Deserialize)]
...
pub struct TranscriptConfig {
    ...
    #[serde(rename = "apiKey")]
    pub api_key: Option<String>,
    /// Default transcription language (e.g., "en", "es", "multi")
...
// NOTE: get_auth_token, get_server_address, and make_api_request have been removed
// Meeting Companion no longer requires a backend server - all operations use SQLx directly
...
/// DEPRECATED: Profile operations require backend server which is no longer used.
/// Meeting Companion stores all data locally via SQLx.
#[tauri::command]
pub async fn api_get_profile<R: Runtime>(
    ...
    log_warn!(
        "api_get_profile called for email: {} - DEPRECATED: Backend not available",
        email
    );
    Err("Profile operations are not available. Meeting Companion operates without a backend server.".to_string())
}
...
pub async fn api_save_profile<R: Runtime>(
    ...
    log_warn!(
        "api_save_profile called for email: {} - DEPRECATED: Backend not available",
        email
    );
    Err("Profile operations are not available. Meeting Companion operates without a backend server.".to_string())
}
...
pub async fn api_update_profile<R: Runtime>(
    ...
    log_warn!(
        "api_update_profile called for email: {} - DEPRECATED: Backend not available",
        email
    );
    Err("Profile operations are not available. Meeting Companion operates without a backend server.".to_string())
}
        ...
        Ok(Some(config)) => {
            ...
            match SettingsRepository::get_transcript_api_key(pool, &config.provider).await {
                Ok(api_key) => {
                    ...
                    // Sync language preference with global state for transcription workers
...
pub async fn api_save_transcript_config<R: Runtime>(
    ...
    if let Some(lang) = language {
        if !lang.is_empty() {
            log_info!("Saving language preference: {}", &lang);
            if let Err(e) = SettingsRepository::save_transcript_language(pool, &lang).await {
                log_error!("Failed to save transcript language: {}", e);
                return Err(e.to_string());
            }
        }
    }
...
/// DEPRECATED: Backend server is no longer required.
/// Meeting Companion operates entirely locally using SQLx for data storage.
...
pub async fn test_backend_connection<R: Runtime>(
    ...
    log_info!("test_backend_connection called - Backend not required, using local SQLx database");
    Ok("Backend not required. Meeting Companion uses local SQLx database for all operations.".to_string())
}
...
/// This function is kept for API compatibility but always returns success.
#[tauri::command]
pub async fn debug_backend_connection<R: Runtime>(_app: AppHandle<R>) -> Result<String, String> {
    log_info!("debug_backend_connection called - Backend not required, using local SQLx database");
    Ok("Backend not required. Meeting Companion uses local SQLx database for all operations. All data is stored in the local SQLite database.".to_string())
}
