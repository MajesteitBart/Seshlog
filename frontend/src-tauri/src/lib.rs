...
pub mod whisper_engine_stub;
#[cfg(target_os = "windows")]
pub use whisper_engine_stub as whisper_engine;
pub mod obsidian;
...
// Global language preference storage (default to "en" for English)
// This is synced with the database setting when transcript config is loaded
...
// Internal helper function to set language preference (for syncing with database config)
pub fn set_language_preference_internal(language: &str) {
    if let Ok(mut lang_pref) = LANGUAGE_PREFERENCE.lock() {
        log_info!("Syncing language preference to: {}", language);
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            // Obsidian file parsing and writing commands
            obsidian::open_meeting_file,
            obsidian::save_meeting_transcript,
            obsidian::save_meeting_markdown_file,
            obsidian::update_meeting_summary,
        ])
