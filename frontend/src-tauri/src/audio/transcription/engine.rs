...
pub async fn validate_transcription_model_ready<R: Runtime>(app: &AppHandle<R>) -> Result<(), String> {
    ...
    match config.provider.as_str() {
        ...
        "deepgram" => {
            info!("🔍 Validating Deepgram configuration...");
            // Deepgram is cloud-based, we just need to verify API key is present
            if let Some(api_key) = config.api_key {
                if api_key.is_empty() {
                    warn!("❌ Deepgram API key is empty");
                    return Err("Deepgram API key is not configured. Please add your API key in Settings > Transcription.".to_string());
                }
                info!("✅ Deepgram API key is configured");
                Ok(())
            } else {
                warn!("❌ Deepgram API key is not set");
                Err("Deepgram API key is not configured. Please add your API key in Settings > Transcription.".to_string())
            }
        }
        other => {
            warn!("❌ Unsupported transcription provider: {}", other);
            Err(format!(
                "Provider '{}' is not supported. Please select 'deepgram', 'localWhisper', or 'parakeet'.",
                other
            ))
        }
    }
    ...
    match config.provider.as_str() {
        ...
        "deepgram" => {
            ...
            let api_key = config.api_key.ok_or_else(|| {
                "Deepgram API key is not configured. Please add your API key in Settings > Transcription.".to_string()
            })?;

            if api_key.is_empty() {
                return Err("Deepgram API key is empty. Please configure it in Settings > Transcription.".to_string());
            }

            // Create Deepgram provider with optional model override
            ...
            let provider = crate::deepgram::DeepgramProvider::with_model(api_key, model.clone());
            info!("✅ Deepgram provider initialized with model '{}'", model);

            // Explicitly coerce to trait object
            let provider: Arc<dyn crate::audio::transcription::provider::TranscriptionProvider> =
                Arc::new(provider);
        ...
        other => {
            warn!("⚠️ Unknown provider '{}', defaulting to Parakeet", other);
            // Re-run with parakeet config
            let engine = {
                let guard = crate::parakeet_engine::commands::PARAKEET_ENGINE
                    .lock()
                    .unwrap();
            ...
            match engine {
                ...
                _ => {
                    Err(format!("Unknown provider '{}' and no fallback engine available.", other))
                }
            }
        }
    }
