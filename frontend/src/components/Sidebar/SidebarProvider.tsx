...
export function SidebarProvider({ children }: { children: React.ReactNode }) {
  ...
  // NOTE: Backend dependency removed - Meeting Companion uses local SQLx database directly
  // serverAddress is kept as a "ready" flag for backwards compatibility with components
  // that check for it before making Tauri invoke calls
  ...
  useEffect(() => {
    // NOTE: Backend dependency removed - Meeting Companion uses local SQLx database directly
    // Set serverAddress to a non-empty value to trigger data fetching
    // The actual URL is not used - all API calls go through Tauri invoke() commands
    const initializeApp = async () => {
      setServerAddress('local'); // Flag to indicate app is ready
      setTranscriptServerAddress('local'); // Kept for backwards compatibility
    };
    initializeApp();
  }, []);
