#!/bin/bash

echo "📄 PRD Status Report"
echo "===================="
echo ""

if [ ! -d ".project/prds" ]; then
  echo "No PRD directory found."
  exit 0
fi

total=$(ls .project/prds/*.md 2>/dev/null | wc -l)
[ $total -eq 0 ] && echo "No PRDs found." && exit 0

# Count by status
draft=0
backlog=0
in_progress=0
review=0
complete=0

for file in .project/prds/*.md; do
  [ -f "$file" ] || continue
  status=$(grep "^status:" "$file" | head -1 | sed 's/^status: *//')

  case "$status" in
    draft|"") ((draft++)) ;;
    backlog) ((backlog++)) ;;
    in-progress|active) ((in_progress++)) ;;
    review) ((review++)) ;;
    complete|completed|done) ((complete++)) ;;
    *) ((draft++)) ;;
  esac
done

echo "Getting status..."
echo ""
echo ""

# Display chart
echo "📊 Distribution:"
echo "================"

echo ""
echo "  Draft:       $(printf '%-3d' $draft) [$(printf '%0.s█' $(seq 1 $((draft*20/total))))]"
echo "  Backlog:     $(printf '%-3d' $backlog) [$(printf '%0.s█' $(seq 1 $((backlog*20/total))))]"
echo "  In Progress: $(printf '%-3d' $in_progress) [$(printf '%0.s█' $(seq 1 $((in_progress*20/total))))]"
echo "  Review:      $(printf '%-3d' $review) [$(printf '%0.s█' $(seq 1 $((review*20/total))))]"
echo "  Complete:    $(printf '%-3d' $complete) [$(printf '%0.s█' $(seq 1 $((complete*20/total))))]"
echo ""
echo "  Total PRDs: $total"

# Recent activity
echo ""
echo "📅 Recent PRDs (last 5 modified):"
ls -t .project/prds/*.md 2>/dev/null | head -5 | while read file; do
  name=$(grep "^name:" "$file" | head -1 | sed 's/^name: *//')
  [ -z "$name" ] && name=$(basename "$file" .md)
  echo "  • $name"
done

# Suggestions
echo ""
echo "💡 Next Actions:"
[ $draft -gt 0 ] && echo "  • Parse draft PRDs to epics: /pm:prd-parse <name>"
[ $in_progress -gt 0 ] && echo "  • Check progress on active PRDs: /pm:epic-status <name>"
[ $total -eq 0 ] && echo "  • Create your first PRD: /pm:prd-new <name>"

exit 0
