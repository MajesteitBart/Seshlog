...
2. **Check for existing tasks:**
   - Check if any numbered task files (001.md, 002.md, etc.) already exist in `.project/epics/$ARGUMENTS/`
...
3. **Validate epic frontmatter:**
   ...
   - If invalid, tell user: "❌ Invalid epic frontmatter. Please check: .project/epics/$ARGUMENTS/epic.md"

4. **Check epic status:**
   - If epic status is already "closed", warn user: "⚠️ Epic is marked as closed. Are you sure you want to decompose it again?"
...
### 1. Read the Epic
- Load the epic from `.project/epics/$ARGUMENTS/epic.md`
...
Task:
  ...
  prompt: |
    ...
    For each task:
    1. Create file: .project/epics/$ARGUMENTS/{number}.md
...
Save tasks as: `.project/epics/$ARGUMENTS/{task_number}.md`
