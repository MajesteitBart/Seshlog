---
description: Merge completed epic from worktree back to main branch
...
2. **Check for active agents:**
   Read `.project/epics/$ARGUMENTS/execution-status.md`
...
feature_list=""
if [ -d ".project/epics/$ARGUMENTS" ]; then
  cd .project/epics/$ARGUMENTS
...
epic_github_line=$(grep 'github:' .project/epics/$ARGUMENTS/epic.md 2>/dev/null || true)
...
# Extract epic issue number
epic_github_line=$(grep 'github:' .project/epics/archived/$ARGUMENTS/epic.md 2>/dev/null || true)
if [ -n "$epic_github_line" ]; then
  epic_issue=$(echo "$epic_github_line" | grep -oE '[0-9]+$' || true)
else
  epic_issue=""
...
# Close task issues
for task_file in .project/epics/archived/$ARGUMENTS/[0-9]*.md; do
  [ -f "$task_file" ] || continue
