...
Follow `.claude/rules/github-operations.md` to ensure we're not syncing to the CCPM template:
...
### For Small Batches (< 5 tasks): Sequential Creation with Rollback

```bash
if [ "$task_count" -lt 5 ]; then
  # Validate epic name to prevent path injection
  if ! echo "$ARGUMENTS" | grep -qE '^[a-zA-Z0-9_-]+$'; then
    echo "❌ Invalid epic name. Only alphanumeric, dash, and underscore allowed."
    exit 1
  fi

  # Create sequentially for small batches with rollback support
  created_issues=()
  ...
  for task_file in ".project/epics/$ARGUMENTS"/[0-9][0-9][0-9].md; do
    ...
    if ! echo "$task_number" | grep -qE '^[0-9]+$'; then
      echo "❌ Failed to create issue for $task_file: $task_number"
      sync_failed=true
      break
    fi

    # Track created issues for potential rollback
    created_issues+=("$task_number")
  ...
  if [ "$sync_failed" = true ] && [ ${#created_issues[@]} -gt 0 ]; then
    echo "⚠️ Sync failed partway through. Rolling back ${#created_issues[@]} created issues..."
    for issue_num in "${created_issues[@]}"; do
      gh issue close "$issue_num" --repo "$REPO" --comment "Rolled back due to sync failure" 2>/dev/null || true
    done
    echo "❌ Sync aborted. Please fix the issue and retry."
    exit 1
  fi
...
while IFS=: read -r task_file task_number; do
  ...
  # Use sed to update the github and updated fields (cleanup .bak files immediately)
  sed -i.bak "/^github:/c\github: $github_url" "$new_name" && rm -f "${new_name}.bak"
  sed -i.bak "/^updated:/c\updated: $current_date" "$new_name" && rm -f "${new_name}.bak"
done < /tmp/task-mapping.txt
```
...
# Update epic frontmatter (cleanup .bak files immediately)
sed -i.bak "/^github:/c\github: $epic_url" ".project/epics/$ARGUMENTS/epic.md" && rm -f ".project/epics/$ARGUMENTS/epic.md.bak"
sed -i.bak "/^updated:/c\updated: $current_date" ".project/epics/$ARGUMENTS/epic.md" && rm -f ".project/epics/$ARGUMENTS/epic.md.bak"
```
...
for task_file in .project/epics/$ARGUMENTS/[0-9]*.md; do
  [ -f "$task_file" ] || continue
...
total_count=$(ls .project/epics/$ARGUMENTS/[0-9]*.md 2>/dev/null | wc -l)
parallel_count=$(grep -l '^parallel: true' .project/epics/$ARGUMENTS/[0-9]*.md 2>/dev/null | wc -l)
...
Create `.project/epics/$ARGUMENTS/github-mapping.md`:
```bash
# Create mapping file
cat > .project/epics/$ARGUMENTS/github-mapping.md << EOF
...
for task_file in .project/epics/$ARGUMENTS/[0-9]*.md; do
  ...
  echo "- #${issue_num}: ${task_name} - https://github.com/${repo}/issues/${issue_num}" >> .project/epics/$ARGUMENTS/github-mapping.md
done

# Add sync timestamp
echo "" >> .project/epics/$ARGUMENTS/github-mapping.md
echo "Synced: $(date -u +"%Y-%m-%dT%H:%M:%SZ")" >> .project/epics/$ARGUMENTS/github-mapping.md
```

### 7. Create Worktree

Follow `.claude/rules/worktree-operations.md` to create development worktree:
...
Follow `.claude/rules/github-operations.md` for GitHub CLI errors.
