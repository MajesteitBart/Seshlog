...
allowed-tools: ListFiles, Read, WriteFile
---
...
2. **Verify PRD exists:**
   - Check if `.project/prds/$ARGUMENTS.md` exists
...
3. **Validate PRD frontmatter:**
   ...
   - If frontmatter is invalid or missing, tell user: "❌ Invalid PRD frontmatter. Please check: .project/prds/$ARGUMENTS.md"
   - Show what's missing or invalid

4. **Check for existing epic:**
   - Check if `.project/epics/$ARGUMENTS/epic.md` already exists
...
5. **Verify directory permissions:**
   - Ensure `.project/epics/` directory exists or can be created
...
### 1. Read the PRD
- Load the PRD from `.project/prds/$ARGUMENTS.md`
...
Create the epic file at: `.project/epics/$ARGUMENTS/epic.md` with this exact structure:
...
- **status**: Always start with "open" for new epics
...
- `.project/epics/$ARGUMENTS/` (directory)
- `.project/epics/$ARGUMENTS/epic.md` (epic file)
...
1. Confirm: "✅ Epic created: .project/epics/$ARGUMENTS/epic.md"
2. Show summary of:
