...
### Status Values
- PRDs: `draft`, `backlog`, `in-progress`, `review`, `complete`
- Epics: `draft`, `backlog`, `in-progress`, `review`, `complete`
- Tasks: `open`, `in-progress`, `review`, `closed`

### Review Enforcement

The `review_required` field enables cascading review enforcement for AI agents:

```yaml
review_required: true    # AI must move to review status before completing
```

**Inheritance Rules (for AI validation):**
- PRD `review_required: true` → inherited by all epics and tasks
- Epic `review_required: true` → inherited by all tasks in that epic
- Task `review_required: true` → applies only to that task

Resolution: Check PRD first, then epic, then task. First `true` wins.

**Note:** This enforcement is for AI agents only. The dashboard UI always shows all status options.
